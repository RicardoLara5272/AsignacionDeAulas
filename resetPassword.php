...
$stmt = $db->prepare('SELECT resetToken, resetComplete FROM docentes WHERE resetToken = :token');
...
if (isset($_POST['submit'])) {
	...
	if (!isset($error)) {
		...
		try {
			...
			$stmt = $db->prepare("UPDATE docentes SET password = :hashedpassword, resetComplete = 'Yes'  WHERE resetToken = :token");
			$stmt->execute(array(
					...
					<div class="login-form">
						...
						<?php if (isset($stop)) {
							...
							<form role="form" method="post" action="" autocomplete="off">
								...
								<hr>
								<div class="row justify-content-center">
									<div class="col-xs-6 col-md-6"><input style="text-transform: uppercase;padding: inherit;" type="submit" name="submit" value="Cambiar Password" class="btn btn-primary btn-block btn-lg" tabindex="3"></div>
								</div>
							</form>
